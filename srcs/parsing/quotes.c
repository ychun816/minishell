...
/*   Updated: 2025/01/07 11:45:39 by varodrig         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 * new_token = creat_token(input_str + 1, len, type,
		content); => skip opening quote (input_str + 1)
 ...
 * 		-> Extracts the content inside the quotes + creates a token for it
			...
			* 		-> Skip the closing quote by moving the index `i` forward by the length of the processed string
			+ 1
 * 		[ELSE] not quotes
...
t_token	*prs_quotes_to_tokens(char *input_str, t_shell *content)
{
   ...
   while (input_str[i])
   {
		...
		{
			new_token =  prs_get_quoted_str(&(input_str[i]), input_str[i],
					content);
			if (!new_token)
					return (NULL);
		...
		{
			new_token = token_create(&(input_str[i]),
					ft_rogue_len(&(input_str[i])), STR, content);
			if (!new_token)
				return (NULL);
			i += ft_strlen(new_token->value) - 1;
		}
